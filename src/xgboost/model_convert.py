...
'''
   ==========================================
   将python训练好的模型转化为PD项目中可用的数组形式 
   ==========================================
'''

'''解析XGBoost.dump模型'''
def load_model(dump_model_path):
    model_fobj = open(dump_model_path,'r')
...
#            print(line)
    ...
    return boosters  
'''将模型转化成数组形式表示'''  
def covert_to_dsp_model(dump_model_path):

    boosters = load_model(dump_model_path)
...
'''将模型数据写成数组形式'''
def write_xgboost_dsp_model_data(dump_model_path,typestr):
    nodeattrs,nodevalues = covert_to_dsp_model(dump_model_path)  
    attrs = '{'
    ...
    values +='}'
    outobj = open('./../../data/model/dsp_model/xgboost_dsp_model_'+typestr+'.txt','w')
...
if __name__ == '__main__':
    dump_model_path = './../../data/model/dump_model/near_surf.model.dump'
    write_xgboost_dsp_model_data(dump_model_path,'near')
#    test()
    #nodeattrs,nodevalues = covert_to_dsp_model()   